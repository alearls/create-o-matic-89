...
const InsightsItem: React.FC<InsightsItemProps> = ({ title, description, onClick, icon }) => {
  return (
    <motion.div 
      className="insight-card cursor-pointer overflow-hidden relative p-3"
      whileHover={{ y: -3, scale: 1.02, boxShadow: "0 10px 25px -5px rgba(103, 71, 246, 0.1), 0 8px 10px -6px rgba(103, 71, 246, 0.05)" }}
    ...
    >
      ...
      <div className="flex flex-col h-[calc(100% - 0.5rem)]">
        {/* Icon positioned at the top */}
        <div className="p-1.5 rounded-full bg-[#6747F6]/10 text-[#6747F6] self-start mb-2">
          {icon}
        </div>
        
        {/* Insight content with more space */}
        <div className="flex flex-col gap-0.5">
          <h4 className="font-medium text-sm text-brand-gray-900">{title}</h4>
          <p className="text-xs text-brand-gray-600">{description}</p>
        </div>
      </div>
    </motion.div>
  );
...
const InsightsPanel: React.FC<InsightsPanelProps> = ({ className }) => {
  ...
  return (
    <motion.div 
      className={cn(
        "h-[calc(100vh-4rem)] w-72 border-l border-brand-gray-200 py-4 px-3 bg-gradient-to-b from-white to-[#F9F8FF]",
        className
      )}
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        <Zap size={18} className="text-[#6747F6]" />
      ...
      <motion.div 
        className="mt-6 p-3 rounded-lg bg-[#6747F6]/5 border border-[#6747F6]/10"
        whileHover={{ scale: 1.02 }}
        transition={{ duration: 0.2 }}
      >
        <h4 className="font-medium text-[#6747F6] mb-1 flex items-center gap-2 text-sm">
          <LightbulbIcon size={14} />
          <span>Pro Tip</span>
        </h4>
        <p className="text-xs text-brand-gray-600">
          Click on any insight card to automatically load it into the chat input, then press Send to analyze.
        </p>
      </motion.div>
    </motion.div>
  );
